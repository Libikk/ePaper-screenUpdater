...
const removeCalendarEventsDuplications = (calendarEvents) => {
  const calendarEventsWithoutDuplicates = calendarEvents.reduce(
    (acc, event) => {
      const isSameSummary = acc.find(
...
const getEventsFromToday = (calendarEvents) => {
  ...
  const eventsFromToday = calendarEvents.filter(({ day }) =>
    isBefore(today, day)
  );
  return eventsFromToday;
};

const calendarEvents = removeCalendarEventsDuplications([
  ...bankHolidaysCalendarEvents,
  ...ePaperCalendarEvents,
]).sort((a, b) => new Date(b.start.date) - new Date(a.start.date));

var days = removeSameDateDuplicates(
  calendarEvents.reduce((acc, event) => {
    const { start, end } = event;
    const dates = getDatesInRange(
      new Date(start.date || start.dateTime),
      new Date(end.date || end.dateTime)
    );
    return [...acc, ...dates];
  }, [])
)
...
const daysWithEvents = getEventsFromToday(
  days.map((day) => {
      ...
      {daysWithEvents.map(({ day, events }) => (
        <div key={day} className="day-container">
          <div className="day">
            {isToday(day) ? (
              <span className="today">
                Today<span className="today-date">{format(day, "do")}</span>
              </span>
            ) : (
          ...
          <div className="event-container">
            {events.map((event) => (
              <div className="event" key={event.summary}>
                {event.summary}
